        ...
        public async Task<IActionResult> RegisterAsync(RegisterRequestModel registerModel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registerResult.Succeeded)
                {
                    ...
                    await _userService.InitializeBaseUserPropertiesAsync(registerModel.UserName);
                ...
                {
                    _logger.LogError("User {username} registration failed", registerModel.UserName);

                    foreach (var error in registerResult.Errors)
                    {
                        responseModel.Errors.Add(error.Description);
                        _logger.LogError("Registration error: {description}", error.Description);
                    }
                }
            }
            else
