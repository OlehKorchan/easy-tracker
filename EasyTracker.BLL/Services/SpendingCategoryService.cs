...
namespace EasyTracker.BLL.Services
{
	public class SpendingCategoryService : ISpendingCategoryService
	{
		...
		public SpendingCategoryService(IUnitOfWork unitOfWork, IMapper mapper)
		{
		...
		public Task<List<MainSpendingCategory>> GetAllMainAsync() =>
			_unitOfWork.MainSpendingCategoryRepository.GetAllAsync();

		public Task<MainSpendingCategory> GetMainAsync(Guid categoryId) =>
			_unitOfWork.MainSpendingCategoryRepository.GetAsync(categoryId);

		public async Task<SpendingCategoryGetDTO> GetAsync(Guid categoryId)
		{
			var spendingCategory = await _unitOfWork.SpendingCategoryRepository.GetAsync(
				categoryId
			);

			var categoryDto = _mapper.Map<SpendingCategoryGetDTO>(spendingCategory);

			categoryDto.Spendings = _mapper.Map<List<SpendingDTO>>(spendingCategory.Spendings);
		...
		public async Task<List<SpendingCategoryGetDTO>> GetAllAsync(string userName)
		{
			var userId = await _unitOfWork.UserRepository.GetUserIdByNameAsync(userName);
			var spendingCategories = await _unitOfWork.SpendingCategoryRepository.GetAllAsync(
				userId
			);

			return spendingCategories.ConvertAll(
				spendingCategory =>
				{
					var categoryDto = _mapper.Map<SpendingCategoryGetDTO>(spendingCategory);

					categoryDto.Spendings = _mapper.Map<List<SpendingDTO>>(
		...
		{
			var currentUserId = (
				await _unitOfWork.UserRepository.GetByNameAsync(spendingCategory.UserName)
			).Id;
		...
		{
			var currentUser = await _unitOfWork.UserRepository.GetByNameAsync(
				spendingCategory.UserName
			);

			var spendingCategoryToDelete = await _unitOfWork.SpendingCategoryRepository.GetAsync(
				spendingCategory.Id
			);
			...
			if (
				!string.Equals(
					currentUser.Id,
					spendingCategoryToDelete.UserId,
